...
def myOrtho(left, right, bottom, top, near, far):
    M_ort = np.array([[2/(right-left), 0, 0, -(right+left)/(right-left)],
                      [0, 2/(top-bottom), 0, -(top+bottom)/(top-bottom)],
                      [0, 0, -2/(far-near), -(far+near)/(far-near)],
                      [0, 0, 0, 1]])
    return glMultMatrixf(M_ort.T)

def myLookAt(eye, at, up):
    ww = eye-at
    w = (ww)/(np.sqrt(np.dot(ww,ww)))
    uu = np.cross(up,w)
    u = (uu)/(np.sqrt(np.dot(uu,uu)))
    v = np.cross(w,u)
    M_v = np.array([[u[0], u[1], u[2], -(np.dot(u,eye))],
                    [v[0], v[1], v[2], -(np.dot(v,eye))],
                    [w[0], w[1], w[2], -(np.dot(w,eye))],
                    [0, 0, 0, 1]])
    return glMultMatrixf(M_v.T)
